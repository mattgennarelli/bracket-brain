...
def calibrate_walk_forward(pairs, objective_name="brier", maxiter=100, popsize=16, seed=42,
    ...
    # PATCHED for full-2026 run: always train the final fit on ALL games,
    # including the most recent complete year (2026), instead of excluding it.
    # This is the ONLY change from the unmodified calibrate.py -- CV search,
    # optimizer settings, param reduction, and gating are all untouched.
    all_train = list(pairs)
    print(f"\n--- Final calibration on {len(all_train)} games (ALL years, including most recent -- PATCHED) ---")
...
def save_config(optimized):
    ...
    PATCHED: writes to calibrated_config_full2026.json, NOT the live
    calibrated_config.json, so this experimental run never overwrites
    the currently-deployed holdout config."""
    out_path = os.path.join(DATA_DIR, "calibrated_config_full2026.json")
